namespace LuaInterface
{
	...
	public class ObjectTranslator
	{
        ...
        /// <summary>
        /// @ ricky pu add
        /// checkCusterm metatable first
        /// </summary>
        internal void checkTypeMetaTable(IntPtr luaState, Type t)
        {
            string metaName = t.AssemblyQualifiedName;
            LuaDLL.luaL_getmetatable(luaState, metaName);
            if (LuaDLL.lua_isnil(luaState, -1))
             {
                if(t.BaseType!=null)checkTypeMetaTable(luaState, t.BaseType);
                 string classStr = "LuaTo" + t.FullName.Replace('.', '_');
                 System.Type metaType = FindType(classStr);
                 if (metaType != null)
                 {
                     object[] arg = new object[] { luaState };
                     metaType.InvokeMember("CreateMetaTableToLua", BindingFlags.Static | BindingFlags.Public | BindingFlags.InvokeMethod, null, null, arg);
                 }
             }
        }
