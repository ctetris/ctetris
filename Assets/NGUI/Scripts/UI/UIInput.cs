...
public class UIInput : MonoBehaviour
{
	...
	protected virtual void Insert (string text)
	{
		...
		int byteLength = System.Text.Encoding.Default.GetByteCount(sb.ToString());
		string ss = sb.ToString();
		if (characterLimit > 0 && byteLength > characterLimit)
		{
			ss = ss.Substring(0, ss.Length - 1);
			byteLength = System.Text.Encoding.Default.GetByteCount(ss);
			while(byteLength > characterLimit)
			{
				ss = ss.Substring(0, ss.Length - 1);
			}
		}
	...
	protected string GetLeftText ()
	{
		...
		min = Mathf.Min(min, mValue.Length);
